...
class PromptStorage:
    ...
    def _load_global_prompts(self) -> Dict[str, dict]:
        ...
        if self.global_prompts_file.exists():
            try:
                ...
                logger.error(f"Failed to load global prompt metadata: {e}")
        return {}
    ...
    def _save_global_prompts(self):
        ...
        try:
            ...
            logger.error(f"Failed to save global prompt metadata: {e}")
    ...
    def _load_user_selections(self) -> Dict[str, dict]:
        ...
        if self.user_selections_file.exists():
            try:
                ...
                logger.error(f"Failed to load user selections: {e}")
        return {}
    ...
    def _save_user_selections(self):
        ...
        try:
            ...
            logger.error(f"Failed to save user selections: {e}")
    ...
    def delete_user_personal_prompt(self, username: str) -> bool:
        ...
        try:
            ...
            if file_path.exists():
                file_path.unlink()
                logger.info(f"Deleted personal prompt file for user {username}")
                return True
            return False
        except Exception as e:
            logger.error(f"Failed to delete personal prompt for user {username}: {e}")
            return False
    ...
    def save_user_personal_prompt(
        ...
        try:
            ...
            logger.error(f"Failed to save user personal prompt: {e}")
            return False
