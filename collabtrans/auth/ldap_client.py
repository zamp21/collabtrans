...
class LDAPClient:
    ...
    def _get_connection(self) -> Connection:
        ...
        if self._connection is None:
            ...
            logger.info(f"Initializing LDAP connection to: {ldap_uri}")
            logger.info(f"LDAP protocol: {self.config.ldap_protocol}")
            logger.info(f"LDAP host: {self.config.ldap_host}:{self.config.ldap_port}")
            ...
            try:
                ...
                if self.config.ldap_protocol == "ldaps":
                    logger.info("Using LDAPS protocol, configuring TLS options")
                    if self.config.ldap_tls_cacertfile:
                        logger.info(f"Setting TLS certificate file: {self.config.ldap_tls_cacertfile}")
                        tls = Tls(
                    ...
                    logger.info("Using LDAP protocol, no TLS configuration needed")
                
                logger.info("LDAP connection initialization completed")
                
            except Exception as e:
                logger.error(f"LDAP connection initialization failed: {e}")
                raise
    ...
    def authenticate(self, username: str, password: str) -> User:
        ...
        logger.info(f"Starting LDAP authentication for user: {_mask_username(username)}")
        logger.info(f"LDAP configuration information:")
        logger.info(f"  - LDAP URI: {self.config.get_ldap_uri()}")
        logger.info(f"  - Protocol: {self.config.ldap_protocol}")
        logger.info(f"  - Host: {self.config.ldap_host}:{self.config.ldap_port}")
        ...
        try:
            ...
            logger.info(f"User search filter: {user_filter}")
            logger.info(f"Search base DN: {self.config.ldap_base_dn}")
            ...
            logger.info(f"Search returned {len(conn.entries)} results")
            if conn.entries:
                logger.info(f"Found user, DN: {conn.entries[0].entry_dn}")
                try:
                    attrs_keys = list(conn.entries[0].entry_attributes_as_dict.keys())
                except Exception:
                    # Compatibility fallback
                    attrs_keys = []
                logger.info(f"User attributes: {attrs_keys}")
            ...
            if hasattr(user_entry, 'displayName') and user_entry.displayName:
                ...
                logger.info(f"User display name: {display_name}")
            elif hasattr(user_entry, 'cn') and user_entry.cn:
                display_name = str(user_entry.cn)
                logger.info(f"User CN: {display_name}")
            ...
            logger.warning("LDAP authentication failed: invalid credentials")
            raise InvalidCredentials("Invalid username or password")
        except LDAPBindError as e:
            logger.warning(f"LDAP bind error: {e}")
            raise InvalidCredentials("Invalid username or password")
        except LDAPException as e:
            logger.error(f"LDAP error: {e}")
            logger.error(f"LDAP error type: {type(e)}")
            logger.error(f"LDAP error details: {str(e)}")
            raise Exception(f"LDAP authentication error: {e}")
        except Exception as e:
            logger.error(f"Exception occurred during authentication: {e}")
    ...
    def _determine_user_role(self, conn: Connection, user_entry) -> UserRole:
        ...
        if not self.config.ldap_admin_group_enabled and not self.config.ldap_glossary_group_enabled:
            logger.info("Group queries not enabled, user defaults to regular user")
    ...
    def _check_admin_group_membership(self, conn: Connection, user_entry) -> bool:
        ...
        try:
            ...
            logger.info(f"Search admin group filter: {admin_group_filter}")
            logger.info(f"Search base DN: {self.config.ldap_group_base_dn}")
            ...
            try:
                ...
                logger.warning(f"Group base DN search failed, trying user base DN: {e}")
                ...
                try:
                    ...
                    logger.info("Base DN search successful")
                except Exception as e2:
                    logger.error(f"Base DN search also failed: {e2}")
                    raise e
            
            if conn.entries:
                admin_group_entry = conn.entries[0]
                logger.info(f"Found admin group: {admin_group_entry.entry_dn}")
                ...
                if hasattr(admin_group_entry, 'member') and admin_group_entry.member:
                    ...
                    logger.info(f"Admin group member count: {len(admin_members)}")
                    
                    # Check if user is in admin group
                    if user_entry.entry_dn in admin_members:
                        logger.info("User is admin group member")
                        return True
            
            logger.info("User is not admin group member")
            return False
            
        except Exception as e:
            logger.error(f"Error occurred during admin group query: {e}")
            logger.warning("Admin group query failed, assuming user is not admin group member")
            return False
    ...
    def _check_user_group_membership(self, conn: Connection, user_entry) -> bool:
        ...
        try:
            ...
            if hasattr(user_entry, 'memberOf') and user_entry.memberOf:
                ...
                for group_dn in member_of_groups:
                    if self.config.ldap_glossary_group.lower() in group_dn.lower():
                        logger.info(f"User is glossary group member: {group_dn}")
                        return True
            
            # If memberOf attribute doesn't exist or no related groups found, determine through group search
            user_group_filter = f"(&(objectClass=group)(cn={self.config.ldap_glossary_group}))"
            logger.info(f"Search glossary group filter: {user_group_filter}")
            logger.info(f"Search base DN: {self.config.ldap_group_base_dn}")
            ...
            if conn.entries:
                ...
                logger.info(f"Found glossary group: {user_group_entry.entry_dn}")
                ...
                if hasattr(user_group_entry, 'member') and user_group_entry.member:
                    ...
                    logger.info(f"Glossary group member count: {len(user_members)}")
                    
                    # Check if user is in glossary group
                    if user_entry.entry_dn in user_members:
                        logger.info("User is glossary group member")
                        return True
            
            logger.info("User is not glossary group member")
            return False
            
        except Exception as e:
            logger.error(f"Error occurred during glossary group query: {e}")
            logger.warning("Glossary group query failed, assuming user is not glossary group member")
            return False
